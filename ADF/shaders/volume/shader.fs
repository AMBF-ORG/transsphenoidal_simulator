...
bool uSmoothVolume = true;
int uSmoothingLevel = 4;
...
vec3 shade(vec3 p, vec3 v, vec3 n, int i)
{
    vec4 lp = gl_ModelViewMatrixInverse * gl_LightSource[i].position;
    vec3 l = normalize(lp.xyz - p * lp.w);
    vec3 h = normalize(l+v);
    float cos_i = max(dot(n, l), 0.0);
    float cos_h = max(dot(n, h), 0.0);

    vec3 Ia = gl_FrontLightProduct[i].ambient.rgb;
    vec3 Id = gl_FrontLightProduct[i].diffuse.rgb * cos_i;
    vec3 Is = gl_FrontLightProduct[i].specular.rgb * pow(cos_h, gl_FrontMaterial.shininess);

    float spotEffect = 1.0;

    // check if spotlight
    if (gl_LightSource[i].spotCutoff < 180.0) {
        vec3 spotDir = normalize(mat3(gl_ModelViewMatrixInverse) * gl_LightSource[i].spotDirection);
        float theta = dot(-l, spotDir);
        float cutoff = cos(radians(gl_LightSource[i].spotCutoff));

        if (theta > cutoff) {
            spotEffect = pow(theta, gl_LightSource[i].spotExponent);
        } else {
            spotEffect = 0.0;
        }
    }
    ...
    float dist = length(gl_LightSource[i].position.xyz - (gl_ModelViewMatrix * vec4(p, 1.0)).xyz);
...
void main(void)
{
    ...
    for (float t = t_entry; t < 0.0; t += t_step, tc += tc_step)
    {
        ...
        if (intensity > uIsosurface)
        {
            ...
            vec3 view = -raydir;
            vec3 colour = (shade(position, view, normal, 0) + shade(position, view, normal, 1)) * texture3D(uVolume, tcr).rgb / uIsosurface;
            sum = vec4(colour, 1.0);

            // vec3 lp = gl_LightSource[1].spotDirection;
